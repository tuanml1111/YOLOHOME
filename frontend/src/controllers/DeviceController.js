...
class DeviceController {
  static async getAllDevices() {
    try {
      ...
      // 🔴 UPDATED: Make sure status is properly mapped as ON/OFF
      return devices.map(device => new DeviceModel({
        id: device.device_id,
        name: device.device_name,
        type: device.device_type,
        location: device.dlocation,
        status: device.status, // This should be 'ON' or 'OFF' from database
  ...
  static async updateDeviceStatus(id, status) {
    try {
      // 🔴 UPDATED: Call the API to update the device status with ON/OFF
      // Ensure status is 'ON' or 'OFF' to match the database schema
    ...
    try {
      ...
      if (response.data && response.data.data) {
        return {
          ...
          // 🔴 UPDATED: Map online/offline to ON/OFF values
      ...
      if (response.data) {
        return {
          ...
          devices: response.data.data ? response.data.data.map(device => new DeviceModel({
            ...
            type: device.device_type,
            location: device.dlocation,
            status: device.status, // This should be 'ON' or 'OFF'
            lastUpdated: device.created_time || new Date().toISOString()
          })) : []
        };
      }
